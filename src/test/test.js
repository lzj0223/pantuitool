...
(function () {
  ...
  <td style="width: 520px"><a href="${url}" target="_blank">${url}</a></td> 
  <td id="${"url-td-" + i}">未处理</td> 
  </tr>
  `
              );
            }
      ...
      },
      {
        ...
        tableChange() {
          ...
          mainButton.onclick = () => {
            backLastPanelTab();
            let targetPath = document.getElementById(
              "default-target-path"
            ).value;
            if (targetPath !== CONFIG.defaultTargetPath) {
              let oldDefaultTargetPath = parseTargetPath(
                CONFIG.defaultTargetPath
              );
  ...
  <tr>
      <td>路径:</td>
      <td><input id="target-path" class="input-base-style" type="text" value="${parseTargetPath(
        CONFIG.defaultTargetPath
      )}"/></td>
  </tr>
  </table>
  ...
  <div>
    <label>默认路径:</label>
    <input id="default-target-path" class="input-base-style" type="text" value="${
      CONFIG.defaultTargetPath
    }">
  </div>
  </div>
  ...
  async function doBatchTransfer(urls, targetPath) {
    ...
    if (targetPath) {
      globalContext.targetPath = targetPath
  ...
  async function verifyPassCode(currContext) {
    ...
    let response = await fetch(
      `${currContext.globalContext.baseUrl}/share/verify?surl=${
        currContext.surl
      }&bdstoken=${
        currContext.globalContext.bdstoken
      }&t=${new Date().getTime()}&channel=chunlei&web=1&clienttype=0`,
      {
        method: "POST",
        referrer: `https://pan.baidu.com/share/init?surl=${currContext.surl}&pwd=${currContext.passCode}`,
        headers: {
